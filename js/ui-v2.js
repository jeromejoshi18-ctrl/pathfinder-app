...
async function loadDevotionHistory() {
  ...
  if (isStudent) {
    ...
    list.innerHTML = sortedPosts.map(([pid, p]) => {
      ...
      const dateLabel = p.tm ? new Date(p.tm).toLocaleDateString(undefined, {weekday:'short', month:'short', day:'numeric'}) : 'Previous Post';
      const authorLabel = p.s ? `Posted by ${p.s}` : 'Instructor Post';
      const textLabel = p.t || 'No message content.';

      return `
        <div class="card" style="margin-bottom:12px;padding:15px">
          <div style="display:flex;justify-content:space-between;margin-bottom:8px">
            <div style="font-size:11px;color:var(--a2);font-weight:800">${dateLabel}</div>
            <div style="font-size:11px;color:var(--mut)">${authorLabel}</div>
          </div>
          <div style="font-size:15px;line-height:1.5;margin-bottom:12px">${textLabel}</div>
    ...
    // For instructors
    list.innerHTML = sortedPosts.map(([pid, p]) => {
      const dateLabel = p.tm ? new Date(p.tm).toLocaleDateString() : 'Unknown Date';
      const textLabel = p.t || '(No text content)';
      ...
      return `
        <div class="card" style="margin-bottom:10px;padding:12px;display:flex;justify-content:space-between;align-items:center">
          <div>
            ...
            <div style="font-size:11px;color:var(--mut);max-width:200px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${textLabel}</div>
          </div>
